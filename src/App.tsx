...
export default function Page() {
  ...
  const [nameOther, setNameOther] = useState(""); // State to store the name value
  
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const nameParam = params.get("name");
    const nameotherTitle = params.get("otherTitle");
    if (nameParam) {
      ...
      const formattednameotherTitle = nameotherTitle ?
        nameotherTitle
          .replace(/-/g, " ") // Replace hyphens with spaces
          .split(" ") // Split the string into words
          .map(word => word.charAt(0).toUpperCase() + word.slice(1)) // Capitalize each word
      ...
      setNameOther(formattednameotherTitle);
      document.title = `${formattedName} be My ❤️?`;
    } else {
      ...
      {yesPressed ? (
        ...
        <>
          ...
          <h1 className="my-4 text-4xl text-center">
            {noCount === 0 || nameOther != "Valentine" ? "" : 
              <div className="text-red-400">
                {" "+getNoButtonText()} <br/>
              </div>
            }
            {name} Will You be My {nameOther ? nameOther : "Valentine"}?
          </h1>
